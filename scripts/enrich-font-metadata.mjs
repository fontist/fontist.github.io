...
//
// Source of truth is vendor/archive-manifest.txt (written by fetch-data.sh
...
// Turn the archive file manifest (one repo-relative path per line) into
// { slug → woff_path } and { slug → coverage_path } maps. Pure and exported so
// the path-shape classification is unit-testable — a regression here silently
// 404s every specimen/coverage asset, and recording a directory instead of a
// real .json was a prior real bug.
//
// woff layouts:      woff/{source}/{slug}.woff            (flat, 3 parts)
//                    woff/{source}/{slug}/{PSName}.woff   (nested, 4 parts)
// coverage layouts:  coverage/{slug}.json                 (legacy flat, 2)
//                    coverage/{source}/{slug}.json        (per-family, 3)
//                    coverage/{source}/{slug}/**/{Face}.json  (per-face, 4+)
// First match per slug wins; coverage entries must always be a real .json.
export function buildIndexesFromManifest(manifest) {
  ...
  return { woffIndex, coverageIndex }
}
...
function main() {
  ...
  const { woffIndex, coverageIndex } = buildIndexesFromManifest(manifest)
...
// Run only when invoked directly (`node enrich-font-metadata.mjs`), not when
// imported by a test — the export above stays side-effect-free.
